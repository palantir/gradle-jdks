...
class JdksPluginIntegrationTest {
    ...
    @Test
    void can_download_run_an_azul_zulu_jdk(GradleInvoker gradle, RootProject rootProject) {
        rootProject.buildGradle().append("""
            javaVersions {
              libraryTarget = 11
            }
    ...
    @Test
    void can_download_run_an_amazon_corretto_jdk(GradleInvoker gradle, RootProject rootProject) {
        rootProject.buildGradle().append("""
            javaVersions {
              libraryTarget = 11
            }
    ...
    @Test
    void can_download_run_on_graal_vm_community_edition_jdk(GradleInvoker gradle, RootProject rootProject) {
        rootProject.buildGradle().append("""
            javaVersions {
              libraryTarget = 23
            }
    ...
    void can_add_ca_certs_to_a_jdk(GradleInvoker gradle, RootProject rootProject) {
            ...
            public final class OutputCaCerts {
                public static void main(String... args) throws Exception {
                    KeyStore keyStore = KeyStore.getInstance(
                     ...
                     System.err.println(Collections.list(keyStore.aliases()));
                     System.out.println(
        ...
        InvocationResult result =
                gradle.withArgs("wrapper", "printCaTruststoreAliases").buildsSuccessfully();

        assertThat(result).output().contains(amazonRootCa1Serial);
    }

    @Test
    void throws_exception_if_there_is_no_jdk_defined_for_a_particular_jdk_major_version(
        ...
        assertThat(result)
                .output()
                .contains("Failed to calculate the value of task ':subproject:compileJava' property 'javaCompiler'.");
        assertThat(result)
                .output()
                .contains("languageVersion=11, vendor=any vendor, implementation=vendor-specific,"
